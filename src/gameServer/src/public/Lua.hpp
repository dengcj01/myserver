...
namespace LuaBind
{
	...
	template <>
	inline char getLuaVal(int idx, lua_State *l) { return (char)lua_tointeger(l, idx); }
	template <>
	inline unsigned char getLuaVal(int idx, lua_State *l) { return (unsigned char)lua_tointeger(l, idx); }
	template <>
	inline short getLuaVal(int idx, lua_State *l) { return (short)lua_tointeger(l, idx); }
	template <>
	inline unsigned short getLuaVal(int idx, lua_State *l) { return (unsigned short)lua_tointeger(l, idx); }
	template <>
	inline long getLuaVal(int idx, lua_State *l) { return (long)lua_tointeger(l, idx); }
	template <>
	inline unsigned long getLuaVal(int idx, lua_State *l) { return (unsigned long)lua_tointeger(l, idx); }
	template <>
	inline int getLuaVal(int idx, lua_State *l) { return (int)lua_tointeger(l, idx); }
	template <>
	inline unsigned int getLuaVal(int idx, lua_State *l) { return (unsigned int)lua_tointeger(l, idx); }
	template <>
	inline long long getLuaVal(int idx, lua_State *l) { return (long long)lua_tointeger(l, idx); }
	template <>
	inline unsigned long long getLuaVal(int idx, lua_State *l) { return (unsigned long long)lua_tointeger(l, idx); }
	template <>
	...
	template <>
	inline std::map<unsigned char, std::string> getLuaVal(int idx, lua_State *l) { return getMapVal<unsigned char, std::string>(idx, l); }
	template <>
	...
	template <>
	inline std::unordered_map<unsigned char, std::string> getLuaVal(int idx, lua_State *l) { return getMapVal1<unsigned char, std::string>(idx, l); }
	template <>
	...
	struct Function_2 : public BaseFunc
	{
		...
		int call(lua_State *l)
		{
			...
			{
				setLuaVal((*func)(getLuaVal<Arg1>(1, l), getLuaVal<Arg2>(2, l)), l);
				return 1;
			}
	...
	struct CallLua
	{
		...
		CallLua(lua_State *l, const char *func, uint8_t retCnt = 0, const char *moduleName = nullptr)
		{
			retCnt_ = retCnt;
			if (!func)
			{
			...
			else
			{
				...
				func_ = func;
				idx_ = luaL_ref(l, LUA_REGISTRYINDEX);
			}
		}
		...
		R call(Args... args)
		{
			...
			else
			{
				...
				//logInfo("callcallcallcallcallcallca1000000000 %d %d", lua_gettop(l), nargs);

				lua_pcall(l, nargs, retCnt_, 1);

				//logInfo("callcallcallcallcallcallca1111111111 %d", lua_gettop(l));
